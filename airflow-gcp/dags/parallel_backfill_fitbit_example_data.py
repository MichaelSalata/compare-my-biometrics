...
@task
def prep_example_profile():
    example_profile_path = "./example_data/profile.json"
    with open(example_profile_path, 'r') as file:
        profile_json = json.load(file)
        if profile_json and profile_json.get("user") and profile_json["user"].get("memberSince"):
            signup_date_str = profile_json["user"]["memberSince"]
            signup_date = datetime.strptime(signup_date_str, "%Y-%m-%d")
            fitbit_hook = FitbitHook()
            return {
                "json_file": fitbit_hook.save_data(data=profile_json, endpoint_name="profile"),
...
def fitbit_pipeline():
    ...
    profile_data = prep_example_profile()
    ...
    def ETL_biometrics(signup_date: datetime, biometric: str):
        biometric_json = f"./example_data/{biometric}.json"
