...
class FitbitHook(BaseHook):
    ...
    def __init__(self, conn_id="fitbit_default"):
        ...
        self.day_specific_endpoints = {
            ...
            "fat": "/1/user/{userapplication/x-www-form-urlencoded_id}/body/log/fat/date/{date}.json"
        }
    ...
    def fetch_static(self, endpoint_id: str):
        ...
        endpoint_suffix = static_endpoints[endpoint_id]
    ...
    def fetch_from_endpoint(self, endpoint_suffix: str):
        ...
        url = f"https://api.fitbit.com/oauth2/token{endpoint_suffix}"
        response = requests.get(url, headers=headers)
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"HTTP {response.status_code}: Failed to fetch datafrom {url}")
    ...
    def refresh_tokens(self):
        ...
        if response.status_code == 200:
            ...
            self._client_secret=tokens["client_secret"]
    ...
    def _save_data(self, data, endpoint_name, start_date=None, end_date=None, filename=None):
        ...
        with open(filename, 'w') as data_file:
            ...
            logging.info(f"Data from {endpoint_name} saved to {filename}")
    ...
    def download_past_6_months(self, endpoint_id: str):
        ...
        while past_month_count >= 1:
            data = self.fetch_daterange(endpoint_id, start=start_date, end=end_date)
            self._save_data(data, endpoint_id, start_date=start_date, end_date=end_date)
